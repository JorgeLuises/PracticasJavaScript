...
//Ejercicio 1:
const movieGenders = (array) => array.reduce((accum, movie) => 
    accum.includes(movie["genre"]) ? accum : [...accum, movie["genre"]], []);
console.log (movieGenders(movies));

//Ejercicio 2:
const getDirectors = (array) => array.reduce((accum, movie) => 
    accum.includes(movie["director"]) ? accum : [...accum, movie["director"]], []);
console.log (getDirectors(movies));

//Ejercicio 3:
const getActors = (array) => {
  let actorsInfo = array.reduce((accum, movie) => {
    let [actor1, actor2] = movie["protagonists"];
    return [...accum, actor1, actor2];
  }, []);
  let actors = actorsInfo.reduce ((accum, actor) => {
    let {name} = actor;
    return accum.includes(name) ? accum : [...accum, name];
  }, []);
  return actors;
};
...
//Ejercicio 4:
const movieRatings = (array) => array.reduce((accum, movie) => 
    accum.includes(movie["rating"]) ? accum : [...accum, movie["rating"]], []);
console.log (movieRatings(movies));

//Ejercicio 5:
const getMovieListDuration = (array, duration1, duration2) => {
  let movieList = array.filter((movie) => 
  parseInt(movie["duration"])>= duration1 && parseInt(movie["duration"])<= duration2 || parseInt(movie["duration"])<= duration1 && parseInt(movie["duration"])>= duration2);
  if (!movieList.length) {
    return `No hay peliculas con duracion entre ${duration1} y ${duration2}`;
  }
  else {
    return movieList;
  }
};
console.log (getMovieListDuration(movies, 100, 120));

//Ejercicio 6:
const getMovieListRating = (array, classification) => {
  let movieList = array.filter((movie) => movie["rating"].toLowerCase() === classification.toLowerCase());
  if (!movieList.length) {
    return `No hay peliculas clasificación ${classification}`;
  }
  else {
    return movieList;
  }
};
console.log (getMovieListRating(movies, "x"));

//Ejercicio 7:
const getMovieListReleaseYear = (array, year1, year2) => {
  let movieList = array.filter((movie) => 
  movie["releaseYear"]>= year1 && movie["releaseYear"]<= year2 || movie["releaseYear"]<= year1 && movie["releaseYear"]>= year2);
  if (!movieList.length) {
    return `No hay peliculas con duracion entre ${duration1} y ${duration2}`;
  }
  else {
...
console.log (getMovieListReleaseYear(movies, 2010, 2005));

//Ejercicio 8:
const getMovieListCountry = (array, pais) => {
  let movieList = array.filter((movie) => movie["country"].toLowerCase() === pais.toLowerCase());
  if (!movieList.length) {
    return `No hay peliculas del país ${pais}`;
  }
  else {
    return movieList;
  }
};
console.log (getMovieListCountry(movies, "México"));

//Ejercicio 9:
const getMovieListOscarAwards = (array) => array.filter((movie) => movie["oscarAwards"] === 0);
console.log (getMovieListOscarAwards(movies));

//Ejercicio 10:
const getNumberOfRatings = (array) => {
  let result = array.reduce((accum, movie) => {
    return accum[movie.rating] ?
    {...accum, [movie.rating]: accum[movie.rating] +1}
    : {...accum, [movie.rating]: 1};
  }, {});
  return result;
};
console.log (getNumberOfRatings(movies));
//Ejercicio 11:
const getNumberOfCountry = (array) => {
  let result = array.reduce((accum, movie) => {
    return accum[movie.country] ?
    {...accum, [movie.country]: accum[movie.country] +1}
    : {...accum, [movie.country]: 1};
  }, {});
  return result;
};
console.log (getNumberOfCountry(movies));
//Ejercicio 12:
const getActorsAge = (array) => {
  let actorsInfo = array.reduce((accum, movie) => {
    let [actor1, actor2] = movie["protagonists"];
    return [...accum, actor1, actor2];
  }, []);
  let result = actorsInfo.reduce((accum,actor) => {
    return accum + actor.age / actorsInfo.length;
  }, 0);
  return result;
};
